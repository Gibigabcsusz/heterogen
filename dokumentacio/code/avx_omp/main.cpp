...
{
	median_filter(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
}

    e0 = clock();
    d0 = (double)(e0-s0)/(CLOCKS_PER_SEC);
	mpixel = (imgWidth*imgHeight/d0)/1000000*RUNS;
    printf("C CPU TIME: %4.4f s\n", d0);
	printf("C 1 RUN:    %4.4f ms\n", d0 * 1000 / RUNS);
...
for (int r=0; r<RUNS; r++)
{
	median_filter_avx_omp(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
}
    e0 = clock();
    d0 = (double)(e0-s0)/(CLOCKS_PER_SEC);
	mpixel = (imgWidth*imgHeight/d0)/1000000*RUNS;
