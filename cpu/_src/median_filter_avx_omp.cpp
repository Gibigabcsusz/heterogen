...
void median_filter_avx_omp(int imgHeight, int imgWidth, int imgWidthF, unsigned char *imgSrcExt, unsigned char *imgDst)
{
		...
		shared(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgDst)
	{
		...
		for (y_out = 0; y_out < imgHeight; y_out++)
		{
			...
			{
				{
					/*prefetch all of the necessary cache lines*/
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 0) * 3 + x_rgb_out), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 0) * 3 + x_rgb_out + 32), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 1) * 3 + x_rgb_out), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 1) * 3 + x_rgb_out + 32), _MM_HINT_NTA);
					

					/* load and sort the first and second 8 long group of values*/

					// load the first 8 values (A00...A07 into regs r00...r07)
					r00 = _mm256_load_si256((__m256i*)(imgSrcExt + imgWidthF * (y_out + 0) * 3 + x_rgb_out + 0 * 3));
					...
					r05 = _mm256_load_si256((__m256i*)(imgSrcExt + imgWidthF * (y_out + 1) * 3 + x_rgb_out + 0 * 3));
					...
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 2) * 3 + x_rgb_out), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 2) * 3 + x_rgb_out + 32), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 3) * 3 + x_rgb_out), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 3) * 3 + x_rgb_out + 32), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 4) * 3 + x_rgb_out), _MM_HINT_NTA);
					_mm_prefetch((char const*)(imgSrcExt + imgWidthF * (y_out + 4) * 3 + x_rgb_out + 32), _MM_HINT_NTA);
					...
					r09 = _mm256_load_si256((__m256i*)(imgSrcExt + imgWidthF * (y_out + 2) * 3 + x_rgb_out + 0 * 3));
					...
					r14 = _mm256_load_si256((__m256i*)(imgSrcExt + imgWidthF * (y_out + 3) * 3 + x_rgb_out + 0 * 3));
					...
					r11 = _mm256_load_si256((__m256i*)(imgSrcExt + imgWidthF * (y_out + 4) * 3 + x_rgb_out + 0 * 3));
                    ...
                    //_mm256_stream_si256((__m256i*)(imgDst + imgWidth * y_out * 3 + x_rgb_out), r05);
					_mm256_store_si256((__m256i*)(imgDst + imgWidth * y_out * 3 + x_rgb_out), r05);
				}
			}
		}
		_mm_free(arr);
...
// stream store:	void _mm256_stream_si256 (__m256i * mem_addr, __m256i a) // Store 256-bits of integer data from a into memory using a non-temporal memory hint. mem_addr must be aligned on a 32-byte boundary or a general-protection exception may be generated.
// prefetch:		void _mm_prefetch (char const* p, int i) // Fetch the line of data from memory that contains address p to a location in the cache heirarchy specified by the locality hint i.
	// for prefetching into non-temporal cache: i = _MM_HINT_NTA